...
def _entry(repo_id: str, revision: str, env: str | None = None) -> tuple[str, str, Path]:
    """Pin a source snapshot; a DOCLING_MLX_*_SOURCE lane variable overrides its location."""
    override = os.environ.get(env) if env else None
    snapshot = HF / f"models--{repo_id.replace('/', '--')}/snapshots" / revision
    return repo_id, revision, Path(override) if override else snapshot
...
PIPELINE_COMPONENTS = frozenset({"pipeline", "pipeline_granite"})
PIPELINE_COMPONENTS = frozenset({"pipeline", "pipeline_granite"})
# Docling's official Granite stages are CPU-only on macOS, so the full pipeline
# uses a reduced page set with a single timed round.  This window is the first
# one that carries both table and chart work.
PIPELINE_GRANITE_PAGES = 50
...
SOURCES: dict[str, tuple[str, str, Path]] = {
    ...
    "table_v1": _entry(
        ...
        "DOCLING_MLX_TABLEFORMER_V1_SOURCE",
    ),
    "table_v2": (
...
def _pipeline_charts(exported: dict[str, Any]) -> list[dict[str, Any]]:
    charts = []
    for picture in exported.get("pictures", []):
        meta = picture.get("meta") or {}
        tabular_chart = meta.get("tabular_chart") or {}
        chart_data = tabular_chart.get("chart_data") or {}
        if not tabular_chart:
            continue
        classification = meta.get("classification") or {}
        predictions = classification.get("predicted_classes") or []
        charts.append(
            {
                "self_ref": picture.get("self_ref"),
                "classification": (predictions[0] or {}).get("class_name") if predictions else None,
                "num_rows": chart_data.get("num_rows"),
                "num_cols": chart_data.get("num_cols"),
            }
        )
    return charts


def _pipeline_snapshot(result: Any, *, include_charts: bool) -> dict[str, Any]:
    ...
    if include_charts:
        snapshot["charts"] = _pipeline_charts(exported)
    return snapshot


def _official_pipeline_artifact_root(
    keys: Iterable[str],
) -> tuple[tempfile.TemporaryDirectory[str], Path]:
    """Expose the pinned official sources under Docling's repository-folder layout."""

    temporary = tempfile.TemporaryDirectory(prefix="docling-backend-pipeline-")
    root = Path(temporary.name)
    for key in keys:
        repo_id, _revision, source = SOURCES[key]
        (root / repo_id.replace("/", "--")).symlink_to(
            _require_directory(source), target_is_directory=True
        )
    return temporary, root


def _mlx_pipeline_artifact_root() -> tuple[tempfile.TemporaryDirectory[str], Path]:
    """Expose the converted MLX mirrors and the official Granite artifact under one root."""

    temporary = tempfile.TemporaryDirectory(prefix="docling-backend-pipeline-mlx-")
    root = Path(temporary.name)
    for entry in sorted(_require_directory(ROOT / ".artifacts").iterdir()):
        if entry.is_dir() and entry.name.startswith("atkinschang--"):
            (root / entry.name).symlink_to(entry.resolve(), target_is_directory=True)
    repo_id, _revision, source = SOURCES["granite"]
    (root / repo_id.replace("/", "--")).symlink_to(
        _require_directory(source), target_is_directory=True
    )
    return temporary, root
...
def _mlx_granite_pipeline_stages() -> dict[str, Any]:
    """Granite table structure, MLX picture classification, and MLX chart extraction."""

    from docling_mlx.stages.chart_extraction import MlxChartExtractionModelOptions
    from docling_mlx.stages.picture_classification import MlxDocumentPictureClassifierOptions
    from docling_mlx.stages.table_structure import MlxGraniteVisionTableStructureOptions
...
def _official_granite_pipeline_stages() -> dict[str, Any]:
    ...
    return {
        "table_structure_options": GraniteVisionTableStructureOptions(),
...
def _pipeline_predictor(
    ...
    granite = component == "pipeline_granite"
    # Docling's official Granite stages accept CPU or CUDA only, so the official
    # pipeline resolves "auto": Torch MPS layout and figure work, Granite on CPU.
    accelerator = AcceleratorOptions(
        device="auto" if granite and implementation != "mlx" else device, num_threads=4
    )
    common: dict[str, Any] = {
        "accelerator_options": accelerator,
        "do_ocr": False,
        "do_chart_extraction": granite,
    ...
    if implementation == "mlx":
        ...
        if granite:
            temporary, artifacts_root = _mlx_pipeline_artifact_root()
            stages = _mlx_granite_pipeline_stages()
            common |= stages
            artifact_metadata |= {
                "table": _source_metadata("granite"),
                "chart": _source_metadata("granite"),
                "max_new_tokens": stages["table_structure_options"].model_spec.max_new_tokens,
                "_temporary": temporary,
            }
        options = build_mlx_pipeline_options(artifacts_root)
    else:
        common["layout_options"] = LayoutObjectDetectionOptions.from_preset("layout_heron_default")
        pipeline_class = StandardPdfPipeline
        temporary, artifacts_root = _official_pipeline_artifact_root(
            ("heron_r50", "figure", "granite") if granite else ("heron_r50", "table_v2", "figure")
        )
        artifact_metadata = {
            "layout": _source_metadata("heron_r50"),
            "table": _source_metadata("granite" if granite else "table_v2"),
            "figure": _source_metadata("figure"),
            "_temporary": temporary,
        }
        if granite:
            common |= _official_granite_pipeline_stages()
            artifact_metadata |= {
                "chart": _source_metadata("granite"),
                "official_stage_max_new_tokens": "tokenizer.model_max_length",
                "device_restriction": "official Docling Granite stages support CPU/CUDA only",
            }
        options = build_official_pipeline_options(artifacts_root)
    artifact_metadata["artifacts_path"] = str(artifacts_root)
    ...
    def predict(record: dict[str, Any]) -> dict[str, Any]:
        ...
        return _pipeline_snapshot(result, include_charts=granite)

    configuration = (
        (
            "layout_heron_default + Granite Vision 4.1 table structure + DocumentFigure "
            "+ Granite Vision 4.1 chart extraction with Docling's default chart tasks; "
            "OCR/code/formula/picture-description disabled"
        )
        if granite
        else (
    ...
    if granite:
        artifact_metadata["page_selection"] = (
            f"first {PIPELINE_GRANITE_PAGES} DPBench pages in manifest order"
        )
    return predict, {"configuration": configuration, **artifact_metadata}
...
def _input_records(
    ...
    if component == "pipeline_granite":
        return list(manifest["pages"][:PIPELINE_GRANITE_PAGES])
    if component == "figure":
...
def _device_label(component: str, implementation: str, device: str) -> str:
    if implementation == "mlx":
        return "mlx-metal"
    if component == "pipeline_granite":
        # Layout and picture classification run on MPS; Docling forces Granite to CPU.
        return "torch-mps+cpu"
...
def _run(args: argparse.Namespace) -> None:
    ...
    if args.component not in PIPELINE_COMPONENTS:
    ...
    if args.component in PIPELINE_COMPONENTS:
    ...
    report = {
        ...
        "device": _device_label(args.component, args.implementation, args.device),
...
def _pipeline_item_count(report: dict[str, Any], key: str) -> int:
    return sum(len(output.get(key, [])) for output in report["outputs"].values())
...
def _timed_round_seconds(report: dict[str, Any]) -> float:
    """Mean wall-clock seconds of one timed round over the whole input set."""

    totals = [sum(item["wall_ms"] for item in round_["items"]) for round_ in report["rounds"]]
    return sum(totals) / len(totals) / 1000.0
...
def summarize(args: argparse.Namespace) -> None:
    ...
    lines = [
        ...
        "three timed rounds over the full input set (one round for official CPU Granite and for "
        "`pipeline_granite`).",
        "",
    ]
    for component in COMPONENTS:
        ...
        if component in GRANITE_COMPONENTS:
        ...
        elif component == "pipeline_granite":
            lines += [
                "| implementation | device | warm ms/item (median) | warm ms/item (mean) | "
                "timed round s | first-call ms | peak RSS | tables | charts | "
                "quality vs official |",
                "| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |",
            ]
            ...
            lines += [
                "| implementation | device | warm ms/item (median) | warm ms/item (mean) | "
                "timed round s | first-call ms | peak RSS | quality vs official |",
                "| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |",
            ]
        else:
            lines += [
        ...
        for implementation, report in reports:
            ...
            if component in PIPELINE_COMPONENTS:
                # Per-page work is uneven (only pages with tables or charts run the table
                # and Granite stages), so the median hides them; report the mean and the
                # timed-round total as well.
                values += [
                    f"{report['warm_per_item_ms']['mean']:.3f}",
                    f"{_timed_round_seconds(report):.1f}",
                ]
            values += [
            ...
            if component in GRANITE_COMPONENTS:
                ...
                values += [
                    str(_pipeline_item_count(report, "tables")),
                    str(_pipeline_item_count(report, "charts")),
                ]
    ...
    for component in COMPONENTS:
        ...
        for implementation in implementations:
            ...
            rounds = (
                1
                if component == "pipeline_granite"
                or (implementation == "official" and component in GRANITE_COMPONENTS)
                else args.rounds
            )
            command = [
                sys.executable,
...
def main() -> None:
    ...
    if args.command == "run" and args.component == "pipeline_granite" and args.rounds != 1:
        parser.error("pipeline_granite runs exactly one timed round")
    args.func(args)
