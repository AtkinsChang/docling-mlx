...
class AvgPoolShortCut(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        padded = mx.pad(
            x,
            [(0, 0), (0, x.shape[1] % 2), (0, x.shape[2] % 2), (0, 0)],
        )
        pooled = self.pool(padded)
        valid = self.pool(
            mx.pad(
                mx.ones((*x.shape[:3], 1), dtype=x.dtype),
                [(0, 0), (0, x.shape[1] % 2), (0, x.shape[2] % 2), (0, 0)],
            )
        )
        return self.proj(pooled / valid)
...
class BottleNeckLayer(nn.Module):
    ...
    def __init__(
        ...
        self.layer = [
            ...
            ConvNormLayer(reduces_channels, out_channels, kernel_size=1, activation=None, eps=eps),
        ]
        self.activation = _resolve_activation(activation)
...
class BasicLayer(nn.Module):
    """RT-DETR ResNet-18/34 two-convolution block."""
    ...
    def __init__(
        ...
        *,
        ...
        if in_channels != out_channels:
            self.shortcut: nn.Module = (
                AvgPoolShortCut(in_channels, out_channels) if should_apply_shortcut else _Identity()
            )
        else:
            self.shortcut = (
                ShortCut(in_channels, out_channels, stride=stride)
                if should_apply_shortcut
                else _Identity()
            )
        self.layer = [
            ConvNormLayer(in_channels, out_channels, stride=stride),
            ConvNormLayer(out_channels, out_channels, activation=None),
        ]
...
class Stage(nn.Module):
    """A ResNet stage with either basic or bottleneck residual blocks."""
...
class Encoder(nn.Module):
    ...
    def __init__(self, config: RtDetrResNetConfig) -> None:
        ...
        for i, (out_c, depth) in enumerate(zip(hidden_sizes, depths, strict=True)):
            if i == 0:
...
class RepVggBlock(nn.Module):
    ...
    def __init__(self, hidden_channels: int, activation: str | None, eps: float) -> None:
        super().__init__()
...
class SinePositionEmbedding(nn.Module):
    ...
    def __call__(self, height: int, width: int, dtype: mx.Dtype = mx.float32) -> mx.array:
        ...
        pe = mx.concatenate([mx.sin(out_h), mx.cos(out_h), mx.sin(out_w), mx.cos(out_w)], axis=1)
        return pe[None, :, :]
...
class EncoderLayer(nn.Module):
    ...
    def __call__(self, x: mx.array, pos_embed: mx.array | None) -> mx.array:
...
class _SelfAttention(nn.Module):
    ...
    def __call__(self, x: mx.array, pos_embed: mx.array | None) -> mx.array:
        ...
        q = self.q_proj(qk).reshape(B, N, self.n_heads, self.head_dim).transpose(0, 2, 1, 3)
        k = self.k_proj(qk).reshape(B, N, self.n_heads, self.head_dim).transpose(0, 2, 1, 3)
...
class AIFI(nn.Module):
    ...
    def __init__(
        self,
        config: RtDetrV2HybridEncoderConfig,
        eval_shape: tuple[int, int] | None,
    ) -> None:
        super().__init__()
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        position_height, position_width = self.eval_shape or (H, W)
        pos = self.position_embedding(position_height, position_width, dtype=x.dtype)
        for layer in self.layers:
        ...
        self.aifi = [
            AIFI(
                ...
                if config.eval_size is None
                    ...
                    config.eval_size[0] // config.feat_strides[level],
                    config.eval_size[1] // config.feat_strides[level],
                ),
            )
...
class VisionTower(nn.Module):
    ...
    This is the model-owned vision module; no mlx-vlm loader shim is retained.
    """

    def __init__(self, config: RtDetrV2Config) -> None:
        super().__init__()
        self.config = config
        # `RtDetrV2Config.__post_init__` resolves backbone_config into a
        ...
        if not isinstance(config.backbone_config, RtDetrResNetConfig):
            raise TypeError(
                ...
                f"RtDetrV2Config.__post_init__, got {type(config.backbone_config).__name__}"
            )
        ...
        self.hybrid_encoder = HybridEncoder(config.hybrid_encoder_config)

    def __call__(self, pixel_values: mx.array) -> tuple[mx.array, ...]:
        backbone = self._backbone_forward or self.backbone
        c_features = backbone(pixel_values)
        proj = tuple(p(c) for p, c in zip(self.encoder_input_proj, c_features, strict=True))
...
def _resolve_activation(name: str | None) -> nn.Module | None:
    if name is None:
        return None
    if name == "relu":
        return nn.ReLU()
