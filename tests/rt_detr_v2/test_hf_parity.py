...
"""End-to-end PekingU RT-DETR-v2 parity against Transformers on CPU."""
...
def _snapshot(name: str) -> Path:
    snapshots = HF_CACHE / f"models--PekingU--{name}" / "snapshots"
    ...
    for candidate in candidates:
        if all((candidate / filename).is_file() for filename in _REQUIRED_FILES):
            return candidate
    pytest.fail(f"RT-DETR-v2 parity requires cached {name}; run: hf download PekingU/{name}")
...
def _images() -> list[Image.Image]:
    ...
    if inputs_value is None:
        pytest.fail(
            "RT-DETR-v2 parity requires DOCLING_MLX_OBJECT_DETECTION_PARITY_INPUTS "
    ...
    if missing:
        pytest.fail(f"RT-DETR-v2 parity requires fixture images: {missing}")
...
@pytest.mark.parametrize(
    "name",
    ("rtdetr_v2_r18vd", "rtdetr_v2_r34vd", "rtdetr_v2_r50vd", "rtdetr_v2_r101vd"),
)
def test_pekingu_checkpoints_match_transformers(name: str) -> None:
    import torch
    from transformers import RTDetrV2ForObjectDetection
    from transformers.models.rt_detr.image_processing_pil_rt_detr import RTDetrImageProcessorPil

    checkpoint = _snapshot(name)
    images = _images()
    native = RtDetrV2Engine(RtDetrV2ModelSpec(path=checkpoint)).predict(images)

    processor = RTDetrImageProcessorPil.from_pretrained(checkpoint)
    reference_model = RTDetrV2ForObjectDetection.from_pretrained(checkpoint).eval()
