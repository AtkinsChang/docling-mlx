...
`release.yml` runs release-please on every push to `main`. It keeps a release pull request open that
...
release, then publishes the tag to PyPI through `publish.yml`, which is a reusable workflow with
...
To rehearse publishing, run `publish.yml` manually from the Actions tab with `testpypi` selected;
